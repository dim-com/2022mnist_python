...
# 有一个警告需要忽略
# 下面import文件里面的
from mydeep.deep_convnet import DeepConvNet
from mysimple.simple_convnet import SimpleConvNet
...
network = SimpleConvNet(conv_param={'filter_num': 30, 'filter_size': 5, 'pad': 0, 'stride': 1})
p1 = module_path + "/mysimple/params.pkl"
p2 = module_path + "/mydeep/deep_convnet_params.pkl"
network.load_params(p1)

# deep_cnn
network1 = DeepConvNet()
network1.load_params(p2)
    ...
    def pbtPredict_3_Callback(self):
        ...
        if self.mod1 < 2:
            ...
            if self.mod1 == 2:
                ...
                __result = network1.predict(img_array)
            # 将预测结果使用softmax输出
